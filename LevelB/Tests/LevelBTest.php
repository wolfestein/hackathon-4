...
class LevelBTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testUltimate($value, $expected)
    {
        ...
        // TEST VITESSE REFERENCE
        $startedAt = microtime(true);
        $a = $this->ref($token);
        $stopedAt = microtime(true);
        $timeRef = ($stopedAt - $startedAt) * 1000000;

        // TEST VITESSE DEVELOPPEUR
        $redux = new Redux($token);

        $startedAt = microtime(true);
        $b = $redux->getReductedNumber();
        $stopedAt = microtime(true);

        $timeDev = ($stopedAt - $startedAt) * 1000000;

        // ON VERIFIE L'EGALITE DES VALEURS
        $this->assertEquals($a, $b);

        // ON VERIFIE QUE PRENDS PLUS DE TEMPS QUE LE DEVELOPPEMENT
        $this->assertGreaterThan($timeDev, $timeRef);
    }
